...
perm_test_parallel_R = function(P, P_2, P_3, P_4, 
  ...
  RES = foreach(cl_id = final_order,
                         ...
                         .errorhandling = "stop") %dorng%{
                           ...
                           .Call(`_distinct_perm_test_parallel`,
                                 P,
                                 P_2,
                                 P_3,
  ...
  p_values_ALL = lapply(RES, function(X){ X[[1]] })
  T_obs = lapply(RES, function(X){ X[[2]] })
  
  # order(final_order) will revert the original oder:
  list(do.call(cbind, p_values_ALL[order(final_order)]),
       do.call(cbind, T_obs[order(final_order)]) )
}
